...
public:
    ...
    int DecideTraj(TxnManager* txMan);
     ...
     */
    RC ExecuteAction(TxnManager* txMan, int action, int prev_action);
    ...
    int GetFeatureTier(const std::string& featureName, double rawValue);

    // 后台监听线程
    void ListenerLoop(int port); // 后台线程的主循环

    uint64_t GetCurrentVersion() {return m_currentVersion.load();}

    void RecordDecisionTraj(TxnManager* txMan, int action, double action_prob, uint32_t policy_version);
...
private:
    ...
    bool ParseJSONProb(const std::string& jsonContent);


    bool ParseUpdateMessage(const std::string& msg);
    std::atomic<uint64_t> m_currentVersion{0}; // 维护当前策略版本
    uint64_t GetCurrentVersion() const { return m_currentVersion.load();}

    std::string ExtractStateKeyTraj(TxnManager* txMan);

    void FinalizeTrajectoryTraj(TxnManager* txMan, bool is_commit);
    ...
    // LDT PPO structure
    std::unordered_map<std::string, std::vector<double>> m_decisionTable_prob;
