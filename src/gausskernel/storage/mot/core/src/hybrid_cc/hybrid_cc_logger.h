...
public:
    ...
    void LogTransactionTrajectory(std::unique_ptr<MOT::TxnTrajectory> traj);
    ...
    void SetNewVersion(uint64_t v) {m_newVersion.store(v);}
    uint64_t GetCurrentVersion() {return m_currentVersion.load();}
    void RotateTrajFile(uint64_t new_version);
...
private:
    ...
    std::atomic<uint64_t> m_newVersion{0};

    // PPO 轨迹日志文件
    std::ofstream m_trajFile;
    std::string m_trajFilepath;
    void WriteTrajHeader(); // 写入轨迹 CSV 表头
