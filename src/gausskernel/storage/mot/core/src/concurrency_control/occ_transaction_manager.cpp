 ...
 */
 ...
 bool OccTransactionManager::ValidateWriteSetPlor(TxnManager* txMan)
 {
     ...
     for (const auto& raPair : orderedSet) {
         ...
         } else if (ac->m_type != INS && txMan->write_lock_rowid_records.count(ac->m_localRow->GetRowId()) == 0) {
             if (!CheckVersion(ac)) {
                 return false;
             }
 ...
 bool OccTransactionManager::GetWriteLockPlor(TxnManager* txMan, uint32_t server_id, void* row)
 {
     ...
     // 提前检验是否上过锁
     if (txMan->write_lock_rowid_records.count(currRow->GetRowId()) != 0) return true;
     txMan->write_lock_rowid_records.emplace(currRow->GetRowId());
 ...
 //            std::string tmp_rowid = currRow->GetTable()->GetTableName() + ":" + to_string(currRow->GetRowId());
             std::string tmp_rowid = currRow->GetTable()->GetLongTableName() + ":" + to_string(currRow->GetRowId());
             // 只对热数据项上锁
             if (hot_rows && txMan->hot_rowid_records.count(currRow->GetRowId()) == 0) continue;
             txMan->write_lock_rowid_records.emplace(currRow->GetRowId());
 ...
 //    if (abort) MOT_LOG_INFO("UnlockWriteSet() txn [Abort] tmp_csn : %s", tmp_csn.c_str());
     for (const auto& raPair : orderedSet) {
         ...
         if (ac->m_type == WR) {
             ...
             if (txMan->write_lock_rowid_records.count(ac->m_localRow->GetRowId()) == 0) {
                 continue;
             }
             ...
             if (txMan->read_lock_rowid_records.count(ac->m_localRow->GetRowId()) == 0) {
                 continue;
             }
