 ...
 */
 ...
 //        EpochCleanVersion();    // 测试
     ...
     static bool InsertTrajToQueue(MOT::TxnManager* txMan);
 ...
 //                MOTAdaptor::epoch_lock_set.insert(row_id, s_tid);     // 先插入当前epoch有锁集, 需要吗?
             ...
             if (writer_.load() == INVALID_TID) {
                 ...
                 if (!writer_list_.empty() && m_writer_.load() == INVALID_TID) {  // 如果队里有request，则取第一个作为grant
                     ...
                     if ((*max_element)->txMan_ != nullptr) m_writer_score_.store((*max_element)->txMan_->GetScore());
         ...
         bool IsSmallerThanWriter(uint64_t& tid, uint64_t& tid_score) {
             ...
             if (it != writer_list_map_.end()) {
                 ...
                 if (writer_request != nullptr && writer_request->txMan_ != nullptr) {
                     uint64_t latest_score = writer_request->txMan_->GetScore();
 ...
 //            hot_rows_set_[tmp_epoch]->clear();      // 清空内容
     ...
     class ActionEngine {
     //    public:
     //        ActionEngine(): env_(ORT_LOGGING_LEVEL_WARNING, "ActionEngine")
     //        {
     //            // 调用预设model
     //            std::string initial_model_path = "/~/RLmodel/model";
     //            // 配置：每次 Run() 只用调用线程
     ...
     //
     //            // 首次用同一路径初始化两个 slot，避免空指针
     //            loadModel(0, initial_model_path);
     //            loadModel(1, initial_model_path);
     //
     //            // input/output 名称（根据你的模型修改）
     //            input_names_  = {"state_input"};
     //            output_names_ = {"action_output"};
     //        }
     //
     //        void loadModel(int idx, const std::string& model_path) {
     //            // load into sessions_[idx]
     //            sessions_[idx].reset(new Ort::Session(env_, model_path.c_str(), session_opts_));
     //        }
     //
     //        // 计算 action
     //        int computeAction(MOT::TxnManager*& txMan, uint64_t& latency) {
     //            int idx = current_idx_.load(std::memory_order_acquire);
     //            auto& session = *sessions_[idx];
     //
     //            // 构建输入 tensor
     //            std::array<float,6> vals = {
     //                static_cast<float>(0.0),
     //                static_cast<float>(txMan->retry_cnt),
     //                static_cast<float>(latency),
     //                static_cast<float>(txMan->read_cnt),
     //                static_cast<float>(txMan->write_cnt),
     //                static_cast<float>(txMan->hot_cnt)
     //            };
     //            Ort::MemoryInfo mem_info = Ort::MemoryInfo::CreateCpu(
     //                OrtAllocatorType::OrtArenaAllocator, OrtMemTypeDefault);
     //            Ort::Value input_tensor = Ort::Value::CreateTensor<float>(
     //                mem_info, vals.data(), vals.size(),
     //                /*dims=*/std::vector<int64_t>{1, 6}.data(), 2);
     //
     //            // Run 推理
     ...
     //            return static_cast<int>(std::round(out[0]));
     //        }
     //        // 禁止拷贝
...
//             // 创建 Env
//             if (api_->CreateEnv(ORT_LOGGING_LEVEL_WARNING, "ActionEngine", &env_) != nullptr) {
//                 // HYBRID_CC: 不强制失败，允许系统继续运行
//                 // assert(false);
//             }
//             // 创建 SessionOptions
//             if (api_->CreateSessionOptions(&session_opts_) != nullptr) {
//                 // HYBRID_CC: 不强制失败，允许系统继续运行
//                 // assert(false);
//             }
//             // 配置线程与优化等级
//             api_->SetIntraOpNumThreads(session_opts_, 1);
//             api_->SetInterOpNumThreads(session_opts_, 1);
//             api_->SetSessionGraphOptimizationLevel(session_opts_, ORT_ENABLE_ALL);
//
             // 预加载两个 slot - 暂时禁用ONNX模型加载
             // const char* initial_model_path = "/~/RLmodel/model.onnx";
             // loadModel(0, initial_model_path);
             // loadModel(1, initial_model_path);
 
             // 名称
//             api_->AllocatorAlloc(nullptr, strlen("state_input") + 1, (void**)&input_name_);
//             strcpy(input_name_, "state_input");
//             api_->AllocatorAlloc(nullptr, strlen("action_output") + 1, (void**)&output_name_);
//             strcpy(output_name_, "action_output");
             // 1. 获取 ONNX Runtime API（添加错误检查）
             api_ = OrtGetApiBase()->GetApi(ORT_API_VERSION);
             if (!api_) {
                 ereport(WARNING,
                     (errmodule(MOD_MOT),
                         errmsg("Failed to get ONNX Runtime API")));
                 return;  // 初始化失败时提前返回
             }

             // 2. 创建环境（修改错误处理）
             OrtStatus* status = api_->CreateEnv(ORT_LOGGING_LEVEL_WARNING, "ActionEngine", &env_);
             if (status || !env_) {
                 const char* err_msg = api_->GetErrorMessage(status);
                 ereport(WARNING,
                     (errmodule(MOD_MOT),
                         errmsg("Failed to create ONNX environment: %s", err_msg ? err_msg : "Unknown error")));
                 api_->ReleaseStatus(status);
                 return;
             }
             // 3. 创建会话选项（添加错误处理）
             status = api_->CreateSessionOptions(&session_opts_);
             if (status || !session_opts_) {
                 const char* err_msg = api_->GetErrorMessage(status);
                 ereport(WARNING,
                     (errmodule(MOD_MOT),
                         errmsg("Failed to create session options: %s", err_msg ? err_msg : "Unknown error")));
                 api_->ReleaseStatus(status);
                 api_->ReleaseEnv(env_);  // 释放已创建的资源
                 env_ = nullptr;
                 return;
             }
             ...
             // 5. 分配输入输出名称（修改内存分配方式）
             input_name_ = strdup("state_input");
             output_name_ = strdup("action_output");

             is_initialized_ = true;  // 标记初始化成功
         }
         ...
         ~ActionEngine() {
             ...
             if (api_) {
                 // 释放会话
                 if (session_[0]) api_->ReleaseSession(session_[0]);
                 if (session_[1]) api_->ReleaseSession(session_[1]);
                 // 释放会话选项
         ...
         void loadModel(int idx, const char* model_path) {
             OrtSession* sess = nullptr;
             OrtStatus* status = api_->CreateSession(env_, model_path, session_opts_, &sess);
             if (status) {
         ...
         int computeAction(MOT::TxnManager* txMan, uint64_t& latency) {
             ...
             api_->CreateCpuMemoryInfo(OrtArenaAllocator, OrtMemTypeDefault, &mem_info);
             ...
             st = api_->Run(
                 sess,
                 nullptr, // OrtRunOptions
                 &input_name_,  &input_tensor, 1,
                 &output_name_, 1, &output_tensor
             );
             ...
             api_->GetTensorMutableData(output_tensor, (void**)&out_data);
             int action = static_cast<int>(std::round(out_data[0]));
 
             api_->ReleaseValue(output_tensor);
             // 切换 slot
             current_idx_.store(1 - idx, std::memory_order_release);
             return action;
         }
