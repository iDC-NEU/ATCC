...
python -m src.ppo_analyze_tiers --update >> process.log 2>&1
if [ $? -ne 0 ]; then
...
python -m src.ppo_pipeline >> process.log 2>&1
if [ $? -ne 0 ]; then
...
echo "$(date '+%Y-%m-%d %H:%M:%S') - train_ppo_process.sh 执行完成" >> process.log
