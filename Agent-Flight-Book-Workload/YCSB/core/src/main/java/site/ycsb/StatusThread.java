...
public class StatusThread extends Thread {
  ...
  // wzy:
  public long lastInteractiveOps = 0;
  public long lastStoredOps = 0;
  public long lastInteractiveRetry = 0;
  public long lastStoredRetry = 0;
  ...
  public void run() {
    ...
    long lastTotalOps = 0;
    lastInteractiveOps = 0;
    lastStoredOps = 0;
    lastInteractiveRetry = 0;
    lastStoredRetry = 0;
  ...
  private long computeStats(final long startTimeMs, long startIntervalMs, long endIntervalMs,
    ...
    long todoops = 0;
    long errorops = 0;
    long totalInteractiveOps = 0;
    long totalStoredOps = 0;
    long totalStoredRetry = 0;
    long totalInteractiveRetry = 0;

    // Calculate the total number of operations completed.
    for (ClientThread t : clients) {
      totalops += t.getOpsDone();
      errorops += t.getOpsError();
      todoops += t.getOpsTodo();
      // wzy:
      totalStoredOps += t.getStoredOpsDone();
      totalStoredRetry += t.getStoredOpsRetry();
      totalInteractiveOps += t.getInteractiveOpsDone();
      totalInteractiveRetry += t.getInteractiveOpsRetry();
    }
    ...
    // wzy:
    double curInteractiveThroughput = 1000.0 * (((double) (totalInteractiveOps - lastInteractiveOps)) /
        ((double) (endIntervalMs - startIntervalMs)));
    double curStoredThroughput = 1000.0 * (((double) (totalStoredOps - lastStoredOps)) /
        ((double) (endIntervalMs - startIntervalMs)));
    double curInteractiveAbortRate = 1000.0 * (((double) (totalInteractiveRetry - lastInteractiveRetry)) / ((double)(totalInteractiveOps - lastInteractiveOps) / 10.0));
    double curStoredAbortRate = 1000.0 * (((double) (totalStoredRetry - lastStoredRetry)) / ((double)(totalStoredOps - lastTotalOps) / 10.0));
    ...
    msg.append(d.format(curInteractiveThroughput)).append(" current interactive ops/sec; ");
    msg.append(d.format(curStoredThroughput)).append(" current stored ops/sec; ");
    msg.append(d.format(curInteractiveAbortRate)).append(" * 100% current interactive abort rate; ");
    msg.append(d.format(curStoredAbortRate)).append(" * 100% current stored abort rate; ");
    lastInteractiveOps = totalInteractiveOps;
    lastStoredOps = totalStoredOps;
    lastInteractiveRetry = totalInteractiveRetry;
    lastStoredRetry = totalStoredRetry;
